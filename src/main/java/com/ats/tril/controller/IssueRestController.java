...
import com.ats.tril.repository.IssueReturnRepository;
...
public class IssueRestController {
	...
	public @ResponseBody List<MrnDetail> updateMrnDetailList(@RequestBody List<MrnDetail> mrnDetailList) {
		...
		try {
			...
			for (int i = 0; i < mrnDetailList.size(); i++) {
				...
				List<MrnDetail> mrnDetailsForUpdate = mrnDetailRepo
						.findByMrnIdAndDelStatus(mrnDetailList.get(i).getMrnId(), 1);
				...
				int updateheaderStatus = mrnHeaderRepository.updateheaderStatus(mrnDetailList.get(i).getMrnId(),
						status);
			}
	...
	/*
	 * select mrn_no from t_mrn_header where mrn_id=(select max(mrn_id) from
	 * t_mrn_header where mrn_date="2018-11-28")
	 */

	/*----------------------------------------------------------*/
	@RequestMapping(value = { "/generateIssueNoAndOfficeMrnNo" }, method = RequestMethod.POST)
	public @ResponseBody ErrorMessage generateIssueNoAndOfficeMrnNo(@RequestParam("docType") int docType,
			@RequestParam("date") String date) {
		...
		try {
			...
			if (docType == 1) {
				...
				recptNo = issueHeaderRepository.generateOfficeMrnNo(date);
			} else {
	...
	@Autowired
	IssueItemListForIssueReturnRepository issueItemListForIssueReturnRepository;

	@RequestMapping(value = { "/getIssueItemListForReturnProcess" }, method = RequestMethod.POST)
	public @ResponseBody List<IssueItemListForIssueReturn> getIssueItemListForReturnProcess(
			@RequestParam("date") String date, @RequestParam("itemIds") List<Integer> itemIds) {

		List<IssueItemListForIssueReturn> list = new ArrayList<>();

		try {

			list = issueItemListForIssueReturnRepository.getIssueItemListForReturnProcess(date, itemIds);
	...
	@RequestMapping(value = { "/saveIssueReturn" }, method = RequestMethod.POST)
	public @ResponseBody List<IssueReturn> saveIssueReturn(@RequestBody List<IssueReturn> issueReturn) {

		List<IssueReturn> res = new ArrayList<>();

		try {

			res = issueReturnRepository.saveAll(issueReturn);
